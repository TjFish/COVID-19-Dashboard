...
function App() {
  ...
  r.useEffect(() => {
    loadCovidData()
      .then((data) => {
        console.log(data)
        setCovidData(data);
        console.log(groupCovidDataByCountries(data));
  ...
  return (
    e('div', null,
      ...
      e('div', {className: 'mb-4',id:'RegionsTable'},
        e(RegionsTable, {
          groupByCountry,
...
function toChina(name)
{
  var uname=name.toUpperCase();
      ...
      {
        return CN_COUNTRIES[uname].translation;
      }
      return name;
}
function getFlag(name)
{
  var uname=name.toUpperCase();
      if(CN_COUNTRIES.hasOwnProperty(uname))
      {
        return 'https://corona.lmao.ninja/assets/img/flags/'+ CN_COUNTRIES[uname].abbreviation.toLowerCase()+".png"
      }
      return '';
}
function RegionsTable({
  ...
  const rows = getCovidRegions(covidData)
    .filter((region) => {
      ...
      return toChina(region.key).search(
    ...
    .map((region, regionIndex) => {
      ...
      return (
        e('tr', {key: toChina(region.key), onClick: () => onRegionChange(region.key)},
          e('td', null, e('input', {type: 'checkbox', checked, onChange: () => {}})),
          e('td', null, e('small', {className: 'text-muted'}, `#${regionIndex + 1}`)),
          e('td', {className: 'text-muted'}, e('img', {src:getFlag(region.key),width:"25px"}),toChina(region.key)),
          //e('td', null, ),
          e('td', {className: 'text-muted'}, confirmedNumber),
          e('td', {className: 'text-muted'}, recoveredNumber),
          e('td', {className: 'text-muted'}, deathsNumber),
